...
var getFileType    = function (file, cb) {
  ...
  magic.detectFile(file, function (err, res) {
    ...
    } else if (res.match(/xml$/i) || res.match(/application\/zip/i)) {
  ...
  return readContents(file)
    .then(function (data) {
      return determineDelimiter(data).then(function (del) {
        ...
        // num cols in each row
        let rowLengths   = rows.map(r => parseRow(r, del).filter(i => i).length);
        // larges number of cols of all rows
        // eslint-disable-next-line no-confusing-arrow
        let maxCols      = rowLengths.reduce((m, c) => c > m ? c : m, 0);
        //get first row with content (to prevent issue with empty first row)
        let firstContent = indexOfFirst(rowLengths);


        let hRows = 0;
        // If the first row with content has the same number rows as the rest of the document
        if (rowLengths.length && rowLengths[firstContent] === maxCols) {
          // choose that row
          hRows = firstContent;
        } else {
          // else there are probably meta headers
          let cols = 0;
          for (let i = 0, l = rowLengths.length; i < l; i++) {
            let next = rowLengths[i];
        ...
        // remove all meta header rows and rejoin.
    ...
    .then(function (data) {
      return determineDelimiter(data).then(function (del) {
        ...
        let rows     = data
          ...
          ;
        // eslint-disable-next-line no-confusing-arrow
        let maxCols      = rows.reduce((max, r) => r.length > max ? r.length : max, 0);

        // eslint-disable-next-line no-confusing-arrow
        rows = rows.map(r => r.length < maxCols ? r.concat(new Array(maxCols - r.length).join('.').split('.')) : r);
